...
const calculateKPIs = (data: any[]): KPI[] => {
  ...
  return [
    { label: 'Total Orders', value: totalOrders.toLocaleString(), change: 'Across India', trend: 'neutral', icon: List, color: 'text-brand' },
    { label: 'Avg Truck Uitilized', value: `${avgUtilization.toFixed(1)}%`, change: '+5.2%', trend: 'up', icon: BarChart2, color: 'text-emerald-500' },
    { label: 'Consolidation Ops', value: consolidationOpportunities.toString(), change: 'Actionable', trend: 'neutral', icon: Layers, color: 'text-brand-dark' },
    ...
    { label: 'Potential Savings', value: `$${totalSavings.toLocaleString(undefined, { maximumFractionDigits: 0 })}`, change: '14.5%', trend: 'up', icon: TrendingUp, color: 'text-brand' },
  ];
};
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
        ...
        <button className="p-2 text-slate-400 hover:text-brand hover:bg-slate-800 rounded-xl transition-all">
                ...
                <tr key={i} className="hover:bg-blue-50/30 transition-all group cursor-default">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex flex-col items-end">
                      <span className="text-brand font-black text-lg leading-none italic tracking-tight">
                        +${savings.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                      </span>
                      <span className="text-[10px] font-bold text-brand uppercase tracking-widest mt-1">
                        Est. Savings
                      </span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button className="bg-white border border-slate-200 text-slate-700 font-bold px-4 py-2 rounded-xl text-xs hover:bg-brand-black hover:text-white hover:border-brand-black transition-all opacity-0 group-hover:opacity-100 transform translate-x-2 group-hover:translate-x-0 flex items-center gap-2 ml-auto shadow-sm">
                      Optimize
        ...
        <div>
          ...
          <div className="flex items-center gap-6">
            <p className="text-sm text-slate-500 flex items-center gap-2">
              Utilization: <span className="font-bold text-brand text-lg">58.8%</span>
            </p>
            <p className="text-sm text-slate-500 flex items-center gap-2 border-l border-slate-200 pl-6">
              Cost/Ton: <span className="font-bold text-brand-black text-lg">$68.2</span>
            </p>
          </div>
        </div>
        <div className="p-3 bg-brand-light text-brand rounded-xl shadow-sm">
          <Activity size={24} />
        </div>
      </div>
          ...
          <AreaChart data={chartData}>
            <defs>
              <linearGradient id="colorUsage" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#CCA23E" stopOpacity={0.15} />
            ...
            />
            <Area yAxisId="left" type="monotone" dataKey="usage" stroke="#CCA23E" strokeWidth={3} fillOpacity={1} fill="url(#colorUsage)" name="Usage %" />
            <Area yAxisId="right" type="monotone" dataKey="cost" stroke="#0a0a0a" strokeWidth={3} fillOpacity={1} fill="url(#colorCost)" name="Cost/Ton" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
...
const OrderOverview = ({ data }: { data: any[] }) => {
  ...
  const COLORS = ['#CCA23E', '#0a0a0a', '#a27f2e', '#d4a843', '#8a6b28'];
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
                  <motion.div
                    ...
                    className="h-full rounded-full" style={{ background: 'linear-gradient(90deg, #CCA23E, #a27f2e)' }}
                  />
                </div>
              </div>
            </div>
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
        <h3 className="text-xl font-bold text-slate-900 mb-4 flex items-center gap-3">
          <div className="p-2 bg-slate-900 rounded-xl">
            <PieIcon size={24} className="text-brand" />
          </div>
    ...
    <div className="bg-slate-50 border-b border-slate-200 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <div className="flex items-center gap-3 mb-2">
          <span className="bg-brand-black text-brand text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider border border-brand/30">
            {shipment.id}
          </span>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white text-brand-black font-sans flex overflow-x-hidden">
      {/* Sidebar Overlay */}
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 h-screen w-72 bg-brand-black border-r border-slate-800 z-50 transform transition-transform duration-300 shadow-2xl lg:shadow-none lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col p-8">
          <div className="flex items-center gap-4 mb-12">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center overflow-hidden">
              <img src="/logo.png" alt="OFI Logo" className="w-full h-full object-contain" />
            </div>
            <div>
              <h1 className="text-2xl font-black text-white tracking-tight">OFI</h1>
              <p className="text-[11px] font-bold text-brand uppercase tracking-widest">Load Consolidation</p>
            </div>
          </div>
          ...
          <div className="mt-auto pt-8 border-t border-slate-800">
            <button
              ...
              className="w-full flex items-center gap-3 p-4 rounded-xl text-slate-400 hover:bg-slate-800 hover:text-brand transition-all duration-300 font-bold group"
            >
              <LogOut size={22} className="text-slate-500 group-hover:text-brand" />
      ...
      <div className="flex-1 flex flex-col min-h-screen w-full relative overflow-x-hidden lg:pl-72">
        {/* Header */}
        <header className="bg-white border-b border-slate-100 sticky top-0 z-30 px-4 md:px-8 lg:px-10 h-20 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            <button className="bg-brand text-white p-2.5 rounded-xl hover:bg-brand-dark transition-colors shadow-lg shadow-brand/20">
              <Package size={22} />
            </button>
          </div>
        </header>
            ...
            {activeTab === 'dashboard' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="bg-brand-black rounded-2xl p-6 md:p-8 text-white relative overflow-hidden shadow-2xl shadow-slate-900/20 flex flex-col justify-center min-h-[300px] lg:min-h-full border border-slate-800">
                    <div className="relative z-10">
                      <h3 className="text-2xl md:text-3xl font-black mb-3 leading-tight text-white italic">Automate Your <br/><span className="text-brand">Logistics</span></h3>
                      <p className="text-slate-400 text-base mb-8 max-w-[320px] leading-relaxed">Run the consolidation engine to find hidden savings in your route data.</p>
                      <button
                        onClick={() => setActiveTab('consolidation')}
                        className="bg-brand text-brand-black px-8 py-3.5 rounded-xl text-base font-bold shadow-xl shadow-brand/20 hover:shadow-2xl hover:bg-white hover:-translate-y-0.5 transition-all flex items-center gap-2 w-fit"
                      >
                        Process Now
                        <ChevronRight size={18} />
                      </button>
                    </div>
                    <Layers size={180} className="absolute -bottom-12 -right-12 opacity-5 rotate-12 hidden md:block text-brand" />
                  </div>
                </div>
                    ...
                    {shipments.length === 0 && !isProcessing && (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="group bg-brand-black hover:bg-brand text-white hover:text-brand-black font-bold py-4 px-10 rounded-xl shadow-xl shadow-slate-900/20 transform hover:-translate-y-1 transition-all flex items-center gap-3 mx-auto text-lg border border-slate-800 hover:border-brand"
                        >
                          Start Consolidation
...
const NavItem = ({ active, icon: Icon, label, onClick, tag }: any) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center justify-between p-4 rounded-xl transition-all duration-300 group ${active ? 'bg-brand/10 text-brand shadow-sm border border-brand/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white border border-transparent'}`}
  >
    <div className="flex items-center gap-3">
      <Icon size={22} className={active ? 'text-brand' : 'text-slate-500 group-hover:text-slate-100'} strokeWidth={2} />
      <span className="text-sm font-bold tracking-tight">{label}</span>
    </div>
    {tag ? (
      <span className={`text-[10px] px-2 py-1 rounded-md ${active ? 'bg-brand/20 text-brand' : 'bg-slate-800 text-slate-400'} font-bold uppercase tracking-wide`}>{tag}</span>
    ) : (
      active && <ChevronRight size={18} strokeWidth={2.5} className="text-brand" />
    )}
  </button>
);
